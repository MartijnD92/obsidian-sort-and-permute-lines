...
export default class MyPlugin extends Plugin {
    ...
    async onload() {
        ...
        this.addCommand({
            id: 'sort-strikethrough',
            name: 'Sort by strikethrough lines (finished tasks)',
            callback: () => this.sortByStrikethrough(),
        })
        this.addCommand({
            id: 'sort-headings',
            name: 'Sort headings',
    ...
    getSortedHeadings(
        ...
        while (currentIndex < lines.length) {
            ...
            if (current.headingLevel) {
                headings.push(
                    this.getSortedHeadings(lines, currentIndex + 1, current)
                )
    ...
    sortByStrikethrough() {
        ...
        lines.sort((a, b) => {
            if (
                strikethroughRegex.test(a.formatted) &&
                !strikethroughRegex.test(b.formatted)
            )
                return 1
            if (
                !strikethroughRegex.test(a.formatted) &&
    ...
    getLines(fromCurrentList = false, ignoreCheckboxes = true): MyLine[] {
        ...
        headings?.map(
            (heading) =>
                (myLines[heading.position.start.line].headingLevel =
                    heading.level)
        )
